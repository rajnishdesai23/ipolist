  ...
  return (
    <section className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-card space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
        ...
        {/* Status Badge */}
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-950/80 dark:text-blue-300 border border-blue-200 dark:border-blue-800 self-start sm:self-auto shadow-sm">
      ...
      <div className="hidden md:block">
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <React.Fragment key={step.key}>
                ...
                {/* Segmented Connector Bar between nodes */}
                {idx < steps.length - 1 && (
                  <div className="flex-1 h-1.5 mx-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 rounded-full ${
                        isNextCompleted || (isCompleted && isNextActive)
        ...
        {/* Row of Step Labels below */}
        <div className="grid grid-cols-4 gap-4 px-2 mt-3 text-center">
          {steps.map((step) => {
            const isCompleted = step.state === "COMPLETED";
            const isCurrent = step.state === "CURRENT";
            ...
            return (
              <div key={step.key} className="flex flex-col items-center">
                <div className="mb-1">
                  {isCompleted ? (
                    <span className="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                      Completed
                    </span>
                  ) : isCurrent ? (
                    <span className="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded bg-blue-600 text-white">
                      Active Today
                    </span>
                  ) : (
                ...
                <span className="font-normal sm:font-medium text-slate-900 dark:text-white block text-sm sm:text-base tracking-tight">
                  {step.label}
                </span>
                <span className="text-xs sm:text-sm font-normal text-slate-500 dark:text-slate-400 mt-0.5 block">
                  {step.dateText}
                </span>
              </div>
            );
